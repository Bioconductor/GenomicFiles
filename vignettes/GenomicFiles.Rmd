...
`r Biocpkg("Rsamtools")` and `r Biocpkg("rtracklayer")`
...
```{r quick_start-load, result="hide", message=FALSE}
...
<a id="fig2" ></a>
[2](#fig2)) split the problem by file. Files are sent to
...
With no REDUCE function, the output is a list the same length as 
the number of ranges where each list element is the length of the number of files.
