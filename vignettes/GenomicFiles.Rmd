...
in `r Biocpkg("GenomicRanges")`, `r Biocpkg("GenomicAlignments")`, `r Biocpkg
("GenomicAlignments")`, `r Biocpkg("Rsamtools")` and `r Biocpkg("rtracklayer")`
...
```{r quick_start-load, message=FALSE}
...
data from the same worker, not across workers. The chart in Figure
[1](reduceByRange_flow.png) represents the division of labor in `reduceByRange`
...
```{r echo = FALSE, fig1, fig.cap=": Mechanics of `reduceByRange` and `reduceRanges`"}
knitr::include_graphics("reduceByRange_flow.png")
```

In contrast to the 'byRange' approach, `reduceByFile` and `reduceFiles` (Figure
[2](reduceByFile_flow.png)) split the problem by file. Files are sent to
different workers with the set of ranges allowing subsets (i.e., multiple
ranges) from the same file to be combined. `reduceByFile` invokes MAP for each
...
```{r echo = FALSE, fig2, fig.cap=": Mechanics of `reduceByFile` and `reduceFiles`"}
