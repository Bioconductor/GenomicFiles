...
in `r Biocpkg("GenomicRanges")`, `r Biocpkg("GenomicAlignments")`, `r Biocpkg("Rsamtools")` 
and `r Biocpkg("rtracklayer")` for an introduction to
these classes.
...
The chart in Figure 
[1](#fig1)
represents the division of labor in `reduceByRange`
...
In contrast to the 'byRange' approach, `reduceByFile` and `reduceFiles` (Figure [2](#fig2)) split the problem by file. Files are sent to
...
`r Biocpkg("deepSNV")` must be loaded on each worker so the function can be found.
...
and performs row-wise $t$-testing using the `rowttests` function from the `rBiocpkg("genefilter")` 
