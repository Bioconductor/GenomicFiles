...
\Rcode{files}, \Rcode{rowRanges}, \Rcode{colData} and \Rcode{exptData} but does
...
MAP extracts and/or manipulates data and REDUCE consolidates the results from MAP.
...
The chart in Figure \ref{reduceByRange_flow} represents the division of labor in
\Rcode{reduceByRange} and \Rcode{reduceRanges} with 3 files and 4 ranges.  These
functions split the problem by range which allows subsets (i.e., the same range)
to be combined across different files.  \Rcode{reduceByRange} invokes MAP for
each range / file combination whereas \Rcode{reduceRanges} invokes MAP a single
time with one file and all ranges.  \Rcode{reduceRanges} passes all files as an
argument to MAP.
...
This division allows subsets (i.e., multiple ranges) to be combined within the
same file.  \Rcode{reduceByFile} invokes MAP for each file / range combination
and \Rcode{reduceFiles} invokes MAP a single time with one range and all files.
