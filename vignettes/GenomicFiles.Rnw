...
The MAPPER function computes pileups for a given range.
<<quick-start-MAPPER>>=
...
<<quick-start-assays>>=
dim(assays(se)[[1]])  ## ranges x files
@

To compute across files instead of within use \Rcode{reduceByRange}.
In this dimension pileup counts from different files can be combined 
for each range with a REDUCER.
...
When \Rcode{summarize=FALSE} (the default) a list is returned. 
<<quick-start-reduceByRange>>=
lst <- reduceByRange(gr, fls, MAPPER, REDUCER, iterate=FALSE)
length(lst)
lst[[1]]
@
...
\subsection{\Rcode{reduceByFile} and \Rcode{reduceByRange}}
...
\subsection{\Rcode{reduceByYield}}

This function is similar in spirit to a one-dimensional \Rcode{reduceByFile}.
Data in a BAM file are chunked through and subject to a \Rcode{MAP}
and \Rcode{REDUCE} function. What sets \Rcode{reduceByYield} apart is the
dependence on \Rcode{yieldSize} in a \Rcode{BamFile} object which 
defines the chunk size and the \Rcode{DONE} argument which defines the 
stop condition.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Chunk and reduce a single file with \Rcode{reduceByYield}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
