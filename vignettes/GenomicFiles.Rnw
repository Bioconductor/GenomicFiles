...
MAPPER <- function(range, file, ...) {
    library(Rsamtools)
    pup <- pileup(file, scanBamParam=ScanBamParam(which=range))
    table(pup[c("nucleotide", "count")])
}
@
...
To compute across files instead of within files use 
\Rcode{reduceByRange}. First define a REDCUCER that adds the 
pileup summary tables.
<<quick-start-REDUCER>>=
...
%% TODO: sum over position
Call \Rcode{reduceByRange}.
<<quick-start-reduceByRange>>=
#se <- reduceByRange(gr, fls, MAPPER, REDUCER, summarize=TRUE)
#se
@
...
can be thought of as a precursor to the \Rcode{SummarizedExperiment} 
class.

\Rcode{GenomicFiles} has slots for \Rcode{files}, \Rcode{rowData}, 
\Rcode{colData} and \Rcode{exptData}. In contrast to 
\Rcode{SummarizedExperiment}, \Rcode{GenomicFiles} does not contain an 
\Rcode{assays} slot and the \Rcode{rowData} is a \Rcode{DataFrame} 
instead of a \Rcode{GenomicRanges}.

The class can be subset on files and / or ranges to perform different
experimental runs. When a \Rcode{GenomicFiles} is given as the 
\Rcode{ranges} argument to the reduceBy* functions and \Rcode{summarize}
is TRUE, data from the common slots are transferred to the resulting 
\Rcode{SummarizedExperiment}.
...
In this example we summarize nucleotide counts (pileups) for the
same ranges across files.
...
apply the reduction iteratively (after MAPPER is done with each range) 
so the function must take 2 arguments representing the `current` 
and `last` results from MAPPER.
...
%% TODO:
\subsection{parallel chunking vs streaming}
