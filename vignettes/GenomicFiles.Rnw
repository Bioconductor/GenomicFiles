...
combined before the next call to \code{MAP} based on whether or not a
\code{REDUCE} is supplied.

\code{reduceRanges} applies \code{MAP} to each range / file combination and 
REDUCEs the output of all MAP calls. \code{REDUCE} usually plays a minor 
...
\Rcode{reduceByFile} invokes \code{MAP} for each file / range combination 
allowing potential \code{REDUCE} after each MAP step.

\Rcode{reduceFiles} applies \code{MAP} to each range / file combination and
REDUCEs the output of all MAP calls. \code{REDUCE} usually plays a minor 
role by concatenating or unlisting results.
